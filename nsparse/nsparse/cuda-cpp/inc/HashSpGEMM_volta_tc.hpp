...
idType SpGEMM_Hash(CSR<idType, valType> a, CSR<idType, valType> b, CSR<idType, valType> &c)
{
    cudaEvent_t event[2];
    float msec;
    for (int i = 0; i < 2; i++) {
   ...
   cudaDeviceSynchronize();
    ...
    idType potential_nnz= elementWiseMultiply<idType, valType>(c,a,d);
    //CSR<IT, VT> f;
    //correct_csr(d,f);
...
idType SpGEMM_Hash(CSR<idType, valType> a, CSR<idType, valType> b, CSR<idType, valType> &c)
{
    idType potential_nnz=SpGEMM_Hash<true, idType, valType>(a, b, c);
    return potential_nnz;
}
