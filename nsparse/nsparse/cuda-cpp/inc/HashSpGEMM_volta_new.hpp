...
template <class idType>
__global__ void hash_symbolic_pwarp(const idType *aia_d_row_1, const idType * aia_d_row_2,
                                    const idType *aia_d_nnz_1, const idType * aia_d_nnz_2,
                                    ...
                                    idType bin_offset, idType M)
{
    ...
    rid = rid + bin_offset;
    nz = 0;
    for (j = aia_d_row_1[rid] + tid; j < aia_d_row_2[rid]; j += PWARP) {
        for (k = aia_d_nnz_1[j]; k < aia_d_nnz_2[j]; k++) {
            bcol = d_bcolids[k];
            key = bcol;
...
__global__ void hash_symbolic_tb(const idType *aia_d_row_1, const idType * aia_d_row_2,
                                 const idType *aia_d_nnz_1, const idType * aia_d_nnz_2,
                                 const idType* __restrict__ d_bcolids,
                                 const idType *d_permutation,
                                 idType *d_row_nz,
                                 idType bin_offset, idType M)
{
    ...
    nz = 0;
    rid = rid + bin_offset;
    for (j = aia_d_row_1[rid] + wid; j < aia_d_row_2[rid]; j += wnum) {
        for (k = aia_d_nnz_1[j] + tid; k < aia_d_nnz_2[j]; k += warp_size_num) {
            bcol = d_bcolids[k];
            key = bcol;
...
template <class idType, int SH_ROW>
__global__ void hash_symbolic_tb_large(const idType *aia_d_row_1, const idType * aia_d_row_2,
                                 const idType *aia_d_nnz_1, const idType * aia_d_nnz_2,
                                       const idType* __restrict__ d_bcolids,
                                       const idType *d_permutation,
                                       idType *d_row_nz,
...
template <class idType>
__global__ void hash_symbolic_gl2(const idType *aia_d_row_1, const idType * aia_d_row_2,
                                 const idType *aia_d_nnz_1, const idType * aia_d_nnz_2,
                                  const idType* __restrict__ d_bcol,
                                  idType *d_row_nz, idType *d_id_table,
                                  ...
                                  idType total_row_num, idType conc_row_num)
{
    ...
    for (; rid < total_row_num; rid += conc_row_num) {
        ...
        for (j = aia_d_row_1[target] + wid; j < aia_d_row_2[target]; j += wnum) {
            for (k = aia_d_nnz_1[j] + tid; k < aia_d_nnz_2[j]; k += warp_size_num) {
                bcol = d_bcol[k];
                key = bcol;
                hash = (bcol * HASH_SCAL) % max_row_nz;
...
template <class idType, class valType>
void hash_symbolic(idType *ad_rpt, CSR<idType, valType> b, CSR<idType, valType> &c, 
                   idType *aia_d_row_1, idType *aia_d_row_2, 
                   idType *aia_d_nnz_1, idType *aia_d_nnz_2, 
    ...
    for (i = BIN_NUM - 1; i >= 0; i--) {
        if (bin.bin_size[i] > 0) {
            ...
            case 0 :
                ...
                hash_symbolic_pwarp<<<GS, BS, 0, bin.stream[i]>>>(aia_d_row_1, aia_d_row_2, aia_d_nnz_1,aia_d_nnz_2, b.d_colids,bin.d_permutation, bin.d_count, bin.bin_offset[i], bin.bin_size[i]);
                cout << "case 0 finish" << endl;
                break;                
            case 1 :
                BS = 64;
                GS = bin.bin_size[i];
            	hash_symbolic_tb<idType, 512><<<GS, BS, 0, bin.stream[i]>>>(aia_d_row_1, aia_d_row_2, aia_d_nnz_1,aia_d_nnz_2, b.d_colids,bin.d_permutation, bin.d_count, bin.bin_offset[i], bin.bin_size[i]);
            	cout << "case 1 finish" << endl;
                break;
            case 2 :
                BS = 128;
                GS = bin.bin_size[i];
            	hash_symbolic_tb<idType, 1024><<<GS, BS, 0, bin.stream[i]>>>(aia_d_row_1, aia_d_row_2, aia_d_nnz_1,aia_d_nnz_2, b.d_colids,bin.d_permutation, bin.d_count, bin.bin_offset[i], bin.bin_size[i]);
            	cout << "case 2 finish" << endl;
                break;
            case 3 :
                BS = 256;
                GS = bin.bin_size[i];
            	hash_symbolic_tb<idType, 2048><<<GS, BS, 0, bin.stream[i]>>>(aia_d_row_1, aia_d_row_2, aia_d_nnz_1,aia_d_nnz_2, b.d_colids,bin.d_permutation, bin.d_count, bin.bin_offset[i], bin.bin_size[i]);
            	cout << "case 3 finish" << endl;
                break;
            case 4 :
                BS = 512;
                GS = bin.bin_size[i];
            	hash_symbolic_tb<idType, 4096><<<GS, BS, 0, bin.stream[i]>>>(aia_d_row_1, aia_d_row_2, aia_d_nnz_1,aia_d_nnz_2, b.d_colids, bin.d_permutation, bin.d_count, bin.bin_offset[i], bin.bin_size[i]);
            	cout << "case 4 finish" << endl;
                break;
            case 5 :
                BS = 1024;
                GS = bin.bin_size[i];
            	hash_symbolic_tb<idType, 8192><<<GS, BS, 0, bin.stream[i]>>>(aia_d_row_1, aia_d_row_2, aia_d_nnz_1,aia_d_nnz_2, b.d_colids, bin.d_permutation, bin.d_count, bin.bin_offset[i], bin.bin_size[i]);
            ...
            case 6 :
                {
            	    ...
            	    hash_symbolic_tb_large<idType, 8192><<<GS, BS, 0, bin.stream[i]>>>(aia_d_row_1, aia_d_row_2, aia_d_nnz_1,aia_d_nnz_2, b.d_colids, bin.d_permutation,bin.d_count, d_fail_count, d_fail_perm, bin.bin_offset[i], bin.bin_size[i]);
            	    ...
            	    if (fail_count > 0) {
            	        ...
            	        GS = bin.bin_size[i];
	                    hash_symbolic_gl<idType><<<GS, BS, 0, bin.stream[i]>>>(ad_rpt, aia_d_nnz_1,aia_d_nnz_2, b.d_colids, d_fail_perm, bin.d_count, d_id_table, max_row_nz, 0, fail_count);
                        cudaFree(d_id_table);
  	                }
                    ...
                    // init_id_table<idType><<<GS, BS, 0, bin.stream[i]>>>(d_id_table, table_size);
                    GS = conc_row_num;
                    hash_symbolic_gl2<idType><<<GS, BS, 0, bin.stream[i]>>>(aia_d_row_1, aia_d_row_2, aia_d_nnz_1,aia_d_nnz_2, b.d_colids, bin.d_permutation, bin.d_count, d_id_table, max_row_nz, bin.bin_offset[i], bin.bin_size[i], conc_row_num);
                    cudaFree(d_id_table);
#endif
                }
                break;
...
__global__ void read_1_new_kernel(idType *res1, idType *res2, idType *res3, idType *res4, const idType *buffer1, const idType *ind1, const idType *buffer2, const idType *ind2, valType n_now) {
    size_t idx = blockIdx.x * blockDim.x + threadIdx.x;
    if (idx < n_now) {
        idType index = ind1[idx];
        res1[idx] = buffer1[index];
        res2[idx] = buffer1[index + 1];
        for (idType j = res1[idx]; j < res2[idx]; j ++) {
           idType index2 = ind2[j];
           res3[j] = buffer2[index2];
           res4[j] = buffer2[index2 + 1]; 
        }
    }
}

template <typename idType, typename valType>
void read_1_new(idType *res1, idType *res2, idType *res3, idType *res4, idType *buffer1, idType *ind1, idType *buffer2, idType *ind2, valType n_now) {
    const int BLOCK_SIZE = 1024;
    int num_blocks = (n_now + BLOCK_SIZE - 1) / BLOCK_SIZE;

    read_1_new_kernel<<<num_blocks, BLOCK_SIZE>>>(res1, res2,res3, res4, buffer1, ind1, buffer2, ind2, n_now);
...
__global__ void elementWiseMultiplyKernel(
    const idType* __restrict__ A_rpt,
    const idType* __restrict__ A_colids,
    const valType* __restrict__ A_values,
    const idType* __restrict__ B_rpt,
    const idType* __restrict__ B_colids,
    const valType* __restrict__ B_values,
    idType* C_rpt,
    idType* C_colids,
    valType* C_values,
    idType nrow)
{
    ...
    idType A_start = A_rpt[row];
    idType A_end = A_rpt[row + 1];
    idType B_start = B_rpt[row];
    idType B_end = B_rpt[row + 1];
    
    idType C_start = C_rpt[row];
    idType C_index = C_start;

    idType a_idx = A_start;
    idType b_idx = B_start;

    while (a_idx < A_end && b_idx < B_end) {
        if (A_colids[a_idx] < B_colids[b_idx] ) {
            a_idx++;
        } else if (A_colids[a_idx] > B_colids[b_idx]) {
            b_idx++;
        } else {
            // Match found, perform multiplication
            C_colids[C_index] = A_colids[a_idx];
            C_values[C_index] = A_values[a_idx] * B_values[b_idx];
            C_index++;
            a_idx++;
            b_idx++;
        }
...
// template <class idType, class valType>
// __global__ void compactNonZeros(
//     const idType* __restrict__ old_rpt,
//     const idType* __restrict__ old_colids,
//     const valType* __restrict__ old_values,
//     idType* new_rpt,
//     idType* new_colids,
//     valType* new_values,
//     idType nrow)
...
//     idType start = old_rpt[row];
//     idType end = old_rpt[row + 1];
//     idType new_start = new_rpt[row];
//     idType count = 0;

//     for (idType i = start; i < end; ++i) {
//         if (old_values[i] != 0) {
//             new_colids[new_start + count] = old_colids[i];
//             new_values[new_start + count] = old_values[i];
//             count++;
//         }
//     }

//     new_rpt[row + 1] = new_start + count;
// }

// template <class idType, class valType>
// void elementWiseMultiply(
//     CSR<idType, valType>& A,
...
//     // Allocate device memory for temporary C
//     idType* d_temp_rpt;
//     idType* d_temp_colids;
//     valType* d_temp_values;
//     cudaMalloc(&d_temp_rpt, (C.nrow + 1) * sizeof(idType));
    
//     // Compute upper bound for C_rpt
//     int block_size = 256;
//     int grid_size = (C.nrow + block_size - 1) / block_size;
//     computeUpperBoundRowPtr<<<grid_size, block_size>>>(A.d_rpt, B.d_rpt, d_temp_rpt, C.nrow);

//     // Perform exclusive scan on temp_rpt to get the row pointers
//     thrust::exclusive_scan(thrust::device, d_temp_rpt, d_temp_rpt + C.nrow + 1, d_temp_rpt);

//     // Get the total number of potential non-zeros in C
//     idType potential_nnz;
//     cudaMemcpy(&potential_nnz, d_temp_rpt + C.nrow, sizeof(idType), cudaMemcpyDeviceToHost);

//     // Allocate memory for temporary C_colids and C_values
//     cudaMalloc(&d_temp_colids, potential_nnz * sizeof(idType));
//     cudaMalloc(&d_temp_values, potential_nnz * sizeof(valType));

//     // Launch kernel for element-wise multiplication
//     elementWiseMultiplyKernel<<<grid_size, block_size>>>(
//         A.d_rpt, A.d_colids, A.d_values,
//         B.d_rpt, B.d_colids, B.d_values,
//         d_temp_rpt, d_temp_colids, d_temp_values,
//         C.nrow
//     );

//     // Allocate device memory for final C
//     cudaMalloc(&C.d_rpt, (C.nrow + 1) * sizeof(idType));
//     cudaMemset(C.d_rpt, 0, (C.nrow + 1) * sizeof(idType));

//     // Compact non-zeros
//     compactNonZeros<<<grid_size, block_size>>>(
//         d_temp_rpt, d_temp_colids, d_temp_values,
//         C.d_rpt, d_temp_colids, d_temp_values,
//         C.nrow
//     );

//     // Perform exclusive scan on C.d_rpt to get the final row pointers
//     thrust::exclusive_scan(thrust::device, C.d_rpt, C.d_rpt + C.nrow + 1, C.d_rpt);

//     // Get the total number of non-zeros in C
//     cudaMemcpy(&C.nnz, C.d_rpt + C.nrow, sizeof(idType), cudaMemcpyDeviceToHost);

//     // Allocate and copy final C_colids and C_values
//     cudaMalloc(&C.d_colids, C.nnz * sizeof(idType));
//     cudaMalloc(&C.d_values, C.nnz * sizeof(valType));
//     cudaMemcpy(C.d_colids, d_temp_colids, C.nnz * sizeof(idType), cudaMemcpyDeviceToDevice);
//     cudaMemcpy(C.d_values, d_temp_values, C.nnz * sizeof(valType), cudaMemcpyDeviceToDevice);

//     // Free temporary memory
//     cudaFree(d_temp_rpt);
//     cudaFree(d_temp_colids);
...
// }
template <class idType, class valType>
void compactNonZeros(
   std::vector<idType>& rpt,
    std::vector<idType>& colids,
    std::vector<valType>& values,
    idType nrow)
{
    idType nnz;
    idType* new_rpt;
    new_rpt = new idType[nrow + 1];
    std::vector<idType> temp_colids;
    std::vector<valType> temp_values;
    ...
    for (idType i = 0; i < nrow; ++i) {
        ...
        for (idType j = start; j < end; ++j) {
            if (values[j] != 0) {
                temp_colids.push_back(colids[j]);
                temp_values.push_back(values[j]);
            }
        }
        
        new_rpt[i + 1] = temp_colids.size();
    }

    nnz = temp_colids.size();
    idType* new_colids = new idType[nnz];
    valType* new_values = new valType[nnz];

    std::copy(temp_colids.begin(), temp_colids.end(), new_colids);
    std::copy(temp_values.begin(), temp_values.end(), new_values);
        std::cout << "Row pointers: ";
    for (int i = 0; i <= std::min((nrow+1), 1000); ++i) {
        std::cout << new_rpt[i] << " ";
    }
    if (nrow > 100) std::cout << "...";
    std::cout << std::endl;

    std::cout << "Column indice: ";
    for (int i = 0; i <= std::min(nnz, 1000); ++i) {
        std::cout << new_colids[i] << " ";
    }
    if (nnz > 100) std::cout << "...";
    std::cout << std::endl;

    std::cout << "Column value: ";
    for (int i = 0; i <= std::min(nnz, 10000); ++i) {
    ...
    CSR<idType, valType>& C)
{
    ...
    elementWiseMultiplyKernel<<<grid_size, block_size>>>(
        ...
        std::vector<idType> h_rpt(A.nrow+1);
cudaMemcpy(h_rpt.data(),d_temp_rpt, (A.nrow+1) * sizeof(idType), cudaMemcpyDeviceToHost);
    std::cout << "Row pointers: ";
    for (int i = 0; i <= std::min((A.nrow+1), 1000); ++i) {
        std::cout << h_rpt[i] << " ";
    }
    if (potential_nnz > 100) std::cout << "...";
    std::cout << std::endl;

    std::vector<idType> h_ind(potential_nnz);
cudaMemcpy(h_ind.data(),d_temp_colids, potential_nnz * sizeof(idType), cudaMemcpyDeviceToHost);
    std::cout << "Column indice: ";
    for (int i = 0; i <= std::min(potential_nnz, 1000); ++i) {
        std::cout << h_ind[i] << " ";
    }
    if (potential_nnz > 100) std::cout << "...";
    std::cout << std::endl;

        std::vector<valType> h_val(potential_nnz);
cudaMemcpy(h_val.data(),d_temp_values, potential_nnz * sizeof(valType), cudaMemcpyDeviceToHost);
    std::cout << "Column value: ";
    for (int i = 0; i <= std::min(potential_nnz, 1000); ++i) {
        std::cout << h_val[i] << " ";
    }
    ...
    // cudaMalloc(&C.d_rpt, (C.nrow + 1) * sizeof(idType));
    // cudaMemset(C.d_rpt, 0, (C.nrow + 1) * sizeof(idType));
     compactNonZeros(h_rpt, h_ind, h_val, C.nrow);
    // Compact non-zeros
    // compactNonZeros<<<grid_size, block_size>>>(
    //     d_temp_rpt, d_temp_colids, d_temp_values,
    //     C.d_rpt, d_temp_colids, d_temp_values,
...
void SpGEMM_Hash(CSR<idType, valType> a, CSR<idType, valType> b, CSR<idType, valType> &c)
{
    ...
    idType *aia_d_nnz_1;
    idType *aia_d_nnz_2;
    cudaMalloc((void**)&aia_d_row_1, a.nrow * sizeof(idType));
    cudaMalloc((void**)&aia_d_row_2, a.nrow * sizeof(idType));
    cudaMalloc((void**)&aia_d_nnz_1, a.nnz * sizeof(idType));
    cudaMalloc((void**)&aia_d_nnz_2, a.nnz * sizeof(idType));
    //read_1_new(aia_d_row,a.d_rpt, bin.d_permutation, nrow);

    read_1_new(aia_d_row_1, aia_d_row_2, aia_d_nnz_1, aia_d_nnz_2, a.d_rpt, bin.d_permutation, b.d_rpt, a.d_colids,a.nrow);
print_partial_data("aia_d_row_1", aia_d_row_1, a.nrow);
print_partial_data("aia_d_row_2", aia_d_row_2, a.nrow);
print_partial_data("aia_d_nnz_1", aia_d_nnz_1, a.nrow);
print_partial_data("aia_d_nnz_2", aia_d_nnz_2, a.nrow);
   //print_bin_sizes(bin);
    ...
    hash_symbolic(a.d_rpt, b, c, aia_d_row_1, aia_d_row_2, aia_d_nnz_1, aia_d_nnz_2, bin, a.nrow);
    ...
    print_csr_matrix(c, "C (Result)");
     elementWiseMultiply<idType, valType>(c,a,d);
    //CSR<IT, VT> f;
    //correct_csr(d,f);
