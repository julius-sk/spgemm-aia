...
std::tuple<long long int, float, float>  SpGEMM_Hash(CSR<idType, valType> a, CSR<idType, valType> b, CSR<idType, valType> &c)
{
...
std::chrono::duration<double, std::milli> duration;
 ...
 cout << "AIA runtime: " << msec << endl;
   ...
   cudaDeviceSynchronize();
    ...
    // cout << "matrix multiply time: " << msec << endl;
    ...
    // cudaDeviceSynchronize();
    //   cudaEventElapsedTime(&msec, eventb[0], eventb[1]);
    //      cout << "matrix copy from device to host: " << msec << endl;
    ...
    //cudaDeviceSynchronize();
    //    cudaEventElapsedTime(&msec, eventb[0], eventb[1]);
     //    cout << "prune_and_scale: " << msec << endl;
    ...
    //cudaDeviceSynchronize();
    //    cudaEventElapsedTime(&msec, eventb[0], eventb[1]);
    //     cout << "normalize_columns: " << msec << endl;
    ...
    // cudaDeviceSynchronize();
    //     cudaEventElapsedTime(&msec, eventb[0], eventb[1]);
    //      cout << "matrix copy from host to device: " << msec << endl;
    ...
    //  std::cout << "result collection Time taken: " << duration.count() / 1000.0 << " ms" << std::endl;
   ...
   std::cout << "total AIA time is \t" << total_AIA_time << std::endl;
   std::cout << "total process time is \t" << total_process_time << std::endl;
    ...
    return std::make_tuple(flop_count, total_AIA_time,total_process_time);
}

template <class idType, class valType>
std::tuple<long long int, float, float> SpGEMM_Hash(CSR<idType, valType> a, CSR<idType, valType> b, CSR<idType, valType> &c)
{
    std::tuple<long long int, float, float> result =SpGEMM_Hash<true, idType, valType>(a, b, c);
    return result;
}
