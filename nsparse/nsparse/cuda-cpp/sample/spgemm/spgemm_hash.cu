...
void spgemm_hash(CSR<idType, valType> a, CSR<idType, valType> b, CSR<idType, valType> &c)
{
    ...
    for (i = 0; i < SpGEMM_TRI_NUM; i++) {
        ...
        if (i > 0) {
            ave_msec += msec;
            total_AIA_time=total_AIA_time+AIA_time;
        }
    }
    ave_msec /= SpGEMM_TRI_NUM - 1;
    total_AIA_time /= SpGEMM_TRI_NUM -1;
    flops = (float)(flop_count) / 1000 / 1000 / ave_msec;
    printf("SpGEMM using CSR format (Hash) without AIA: %f[GFLOPS], %f[ms]\n", flops, ave_msec);
    printf("average aia time is %f[ms]\n", total_AIA_time);
    flops = (float)(flop_count) / 1000 / 1000 / (ave_msec-total_AIA_time);
    printf("SpGEMM using CSR format (Hash): %f[GFLOPS], %f[ms]\n", flops, ave_msec-total_AIA_time);
